...
you can draw or remove frames in `stepcap edit` (`--marker ring` always uses rings). Drags
get an arrow, scrolls a direction arrow, shortcuts a key-cap label. `--zoom 640` makes the
crop around the click the main image and adds a full-screen thumbnail:
