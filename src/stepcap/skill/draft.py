...
if TYPE_CHECKING:
    from stepcap.skill.merge import Variants
...
NOTES_FOR_AGENT = (
    ...
    "The recorded runs are examples. Names, dates, positions and window titles will differ - "
    "match by meaning, and stop and ask when the screen does not look like the description.",
)
...
def render(
    ...
    ``variants`` (several recordings, RM-068) adds optional steps, the values typed in
    each recording and the steps only other recordings had."""
    many = variants is not None and variants.total > 1
    ...
    runs = f" (from {variants.total} recordings)" if many else ""
    description = (
    ...
    head = frontmatter.dump(
        {
            ...
            "metadata": {
                ...
                "status": "draft",
                **({"recordings": str(variants.total)} if many else {}),
            },
        }
    )

    out = [head, f"# {title}", ""]
    source = f"{variants.total} recordings (the first is shown)" if many else "one recording"
    out += [
        f"> Draft generated by stepcap from {source}. Check the Goal and Inputs, then",
        "> generalise the steps (or run `stepcap skill --agent claude|codex|gemini`).",
        "",
        "## Goal",
        "",
    ]
    ...
    for n, s in enumerate(steps, 1):
        ...
        if inp.get("variable", True) or not example:
            ...
            seen = _examples(variants, s["id"]) if many else []
            if len(seen) > 1:
                line += "; typed in the recordings: " + ", ".join(f"`{x}`" for x in seen)
            elif seen and len(variants.values[s["id"]]) == variants.total:
                line += f"; the same in all {variants.total} recordings: `{seen[0]}`"
                line += " (maybe a fixed value)"
            elif example:
    ...
    for n, s in enumerate(steps, 1):
        ...
        if many and variants.seen.get(s["id"], variants.total) < variants.total:
            sub.append(f"Optional: done in {variants.seen[s['id']]} of {variants.total} recordings")
        if s["id"] in refs:
...
def _examples(variants: Variants, sid: str) -> list[str]:
    seen: dict[str, None] = {}
    for v in variants.values.get(sid, []):
        if _one_line(v):
            seen[_one_line(v)] = None
    return list(seen)


def _differences(variants: Variants) -> list[str]:
    out = ["", "## Differences between recordings", ""]
    names = ", ".join(f"{n}. `{_one_line(src)}`" for n, src in enumerate(variants.sources, 1))
    out.append(f"Recordings: {names}. The steps above are from recording 1.")
    out.append("")
    if not variants.extra:
        out.append("The other recordings did no step that recording 1 did not.")
        return out
    for e in variants.extra:
        where = _where(e.step)
        at = f"after step {e.after}" if e.after else "before step 1"
        line = f"- Recording {e.recording}, {at}: **{_one_line(e.step.get('title')) or 'Step'}**"
        out.append(line + (f" ({where})" if where and e.step.get("kind") != "manual" else ""))
    return out
